...
from yolo_models.processing.pose_processing import YoloPoseDetectorProcesing
...
def main(args):
    ...
    logger.info(f"🚀 YOLO Server: {args.checkpoint_path} | 📐 {args.image_width}x{args.image_height} | 💾 mem={args.shared_array_mem_name}")

    # Determine if this is a pose model based on filename
    is_pose_model = "pose" in os.path.basename(args.checkpoint_path).lower()
    
    if is_pose_model:
        # Pose processor has different argument order
        with YoloPoseDetectorProcesing(args.shared_update_mem_name,
        ...
        # Detection processor has model path first
...
if __name__ == "__main__":
    ...
    parser.add_argument("-ih", "--image_height", type=int, default=720,
