def setup_conda_environment():
    ...
    conda_env_path = os.path.expanduser("~/miniconda3/envs/yolo_env")
    
    # Add the conda environment's site-packages (Python 3.11)
    site_packages = os.path.join(conda_env_path, "lib", "python3.11", "site-packages")
    
    if site_packages not in sys.path:
        sys.path.insert(0, site_packages)
        print(f"Added conda site-packages to path: {site_packages}")
    
    # Also add the project directory for local yolo_models
    project_dir = os.path.expanduser("~/work/code/yolo-video-detection-example")
    ...
    print(f"Python paths: {sys.path[:3]}...")  # Show first 3 paths
    
    return project_dir, site_packages

# Setup conda environment paths first
try:
    project_dir, site_packages = setup_conda_environment()
