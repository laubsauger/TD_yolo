...
def restore_original_coordinates_inplace(bbox_xywh: BboxType, pad_info: PadInfo) -> BboxType:
    bbox_xywh[:, 0] -= pad_info.pad_left
    bbox_xywh[:, 1] -= pad_info.pad_top
    return bbox_xywh


def yolo_bbox2xywh_inplace(yolo_xywh_bboxes: BboxType) -> BboxType:
    yolo_xywh_bboxes[:, 0] -= yolo_xywh_bboxes[:, 2] / 2
    yolo_xywh_bboxes[:, 1] -= yolo_xywh_bboxes[:, 3] / 2
    return yolo_xywh_bboxes

def xyhw2xyxy_inplace(xy2wh: BboxType) -> BboxType:
    xy2wh[:, 2] += xy2wh[:, 0]
    xy2wh[:, 3] += xy2wh[:, 1]
    return xy2wh

def clip_boxes_inplace(boxes: np.ndarray, width: int, height: int) -> None:
    """
...
def scale_bbox_inplace(bbox: BboxType, scale_info: ScaleInfo) -> BboxType:
