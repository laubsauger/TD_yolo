...
class ResizeToRequiredSize(nn.Module):
    ...
    def forward(self, image: torch.Tensor) -> Tuple[torch.Tensor, ScaleInfo]:
        ...
        # Disable antialias on MPS as it's not supported yet
        if image.device.type == 'mps':
            return F.resize(image, [new_height, new_width], interpolation=InterpolationMode.BILINEAR, antialias=False), ScaleInfo(width / new_width, height / new_height)
        else:
