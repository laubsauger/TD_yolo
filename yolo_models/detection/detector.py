...
class Detector(ABC):
    ...
    def postpocess(self,
        ...
        """pred: [1 x 84 x 8400] for detection models or [1 x 56 x 8400] for pose models

            1 - bath size
            84 - 0,1,2,3 is x,y,width,height, 4,5,6,7,8,9.... probability for each class
            56 - 0,1,2,3 is x,y,width,height, 4 is confidence, 5-55 are keypoints
        ...
        # Check if this is a pose model by the shape
        if pred.shape[1] == 56:  # Pose model
            # For pose models, confidence is at index 4
            scores = pred[0, 4, :]
            raw_scores = pred[0, 4:5, :]  # Just wrap confidence as single "class"
        ...
        # For pose models, all detections are "person" (class 0)
        if pred.shape[1] == 56:  # Pose model
            class_indices = np.zeros(len(bbox_indices), dtype=int)
        else:  # Detection model
        ...
        return DetectionInfo(
            scores[bbox_indices],
            det_xyxy,
            [class_mapping.get(int(class_index), f"class_{int(class_index)}") for class_index in class_indices]
        )
...
class PyTorchYoloDetector(Detector):
    def __init__(self,
        ...
        self._is_pose_model = (
            (hasattr(self._model, 'task') and self._model.task == 'pose') or
            (hasattr(self._model, 'yaml') and 'kpt_shape' in self._model.yaml)
        )
        if self._is_pose_model:
            self._logging.info("Detected pose model")
    ...
    def _raw_predict(self, image: torch.Tensor):
        ...
        # Store raw output for pose models (before NMS)
        self._last_raw_output = out
    ...
    def _torch_postpocess(self,
        ...
        # For pose models, all detections are "person" (class 0)
        if pred.shape[1] == 56:  # Pose model
            class_indices = torch.zeros(len(bbox_indices), dtype=torch.long)
        else:  # Detection model
